#!/usr/bin/env node
/**
 * Fetches entry-level (0–2 yrs) Human Resources jobs in Singapore
 ...
 *      experience + positionLevels enforced client-side.
 *
 ...
 *
 * Writes deduplicated results to docs/hr_jobs/README.md
...
const OUTPUT_PATH = process.env.OUTPUT_PATH ?? 'docs/hr_jobs/README.md';
...
async function fetchMyCareersFuture() {
  ...
  while (true) {
    ...
    url.searchParams.set('search', 'human resources');
    ...
    for (const job of jobs) {
      ...
      results.push({
        title: job.title ?? 'Human Resources',
...
async function fetchLinkedIn() {
  ...
  const url =
    'https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search' +
    '?keywords=human+resources&location=Singapore&f_E=2&f_TPR=r172800&start=0';
...
function formatMarkdown(jobs, counts) {
  ...
  return `# Singapore Entry-Level Human Resources Jobs
...
// ---------------------------------------------------------------------------
async function main() {
  console.log(`Fetching HR jobs… (cutoff: ${CUTOFF.toISOString()})`);
